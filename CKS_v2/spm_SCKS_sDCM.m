...
%--------------------------------------------------------------------------
Sx      = sparse(real(spm_sqrtm(M(1).xP)));             % state prior variance
sQ      = sparse(real(spm_sqrtm(inv(M(1).W))));         % state noise variance

if trace(M(1).uP)~=0
    Su  = sparse(real(spm_sqrt(M(1).uP)));            %input prior variance
...
if ~isempty(ip)
    ...
    sW      = cell(1,T-1);                         % robbins munroe updates for this !
...
for run = 1:RUN
    ...
    for t = 2:T,
        ...
        % robbin's munroe update for parametre noise w.  
        if ~isempty(ip)
            d      = 1/ap1(1);
            subKG  = K(wind,:);
            try
        ...
        % annealing step for state noise q
        d2     = 1/ap2(1);
        subKG  = K(xind,:);
        try
        ...
        ap2(1)                 = min(ap2(1)+0.001,ap2(2)); % slowly reduce the annealing factors
